...
export const fetchAttemptTests = (profileID) => async (dispatch) => {
  ...
  await fetch(`http://localhost:3000/student/attempt-tests/${profileID}`, requestOptions)
...
export const fetchTests = (className) => async (dispatch) => {
  ...
  await fetch(`http://localhost:3000/student/tests/${className}`, requestOptions)
    .then((response) => response.json())
    .then((data) => {
      if (data?.error?.name === "TokenExpiredError") {
        dispatch(logoutUser());
...
export const submitTest = (data) => async (dispatch) => {
  ...
  await fetch(`http://localhost:3000/student/submit-test/${testID}`, requestOptions)
    .then((response) => response.json())
    .then((data) => {
      if (data) {
        if (data?.error?.name === "TokenExpiredError") {
...
export const fetchTeacherTests = (profileID) => async (dispatch) => {
  ...
  await fetch(`http://localhost:3000/teacher/tests/${profileID}`, requestOptions)
    .then((response) => response.json())
    .then((data) => {
      if (data) {
        if (data?.error?.name === "TokenExpiredError") {
